 ...
 * Some vocabulary
 * ---------------
 * * (Acumulus) invoice: the representation of invoice data as presented to Acumulus web service API and that can be used to create a new invoice in Acumulus.
 * * Shop invoice: The webshop's own representation of an invoice. Not all webshops support a fully fledged notion and storage of an invoice.
 *   Many see an order with price (and tax) info as an invoice.
 * * Source or invoice source: a webshop object that can serve as a source for the creation of an (Acumulus) invoice.
 *   This library supports orders and refunds (credit memos) as invoice source, the latter of course only as far as supported by the webshop itself.
 * * Extension: a way to extend the functionality of a webshop or CMS. Other words used for this concept are module, addon, plugin, and component.
 * * User: a person with administrative access rights to an installation of the webshop or CMS system. Clients/buyers/visitors are thus not users and this library will
 *   never present messages or any other texts to visitors.
 *
 * Introduction
 * ------------
 * LibAcumulus was written to simplify development of client side code that communicates with the Acumulus web service and that resides within the ecosystem
 * of a webshop or CMS with a webshop extension. Therefore this library does not:
 * * Contain controller like classes as they normally have to fit within the webshop's architecture.
 * * Directly output forms, views or messages as normally webshop specific ways to do so exist.
 * * Contain explicit install or update code as normally webshop specific ways to invoke this exists. However, it does contain code that can be called by the webshop's install or update code.
 *
 ...
 * * {@see \Siel\Acumulus\Shop}: Entity handling, form handling, and managing (controller like) classes.
 * * {@see \Siel\Acumulus\Web}: web service and communication related classes.
 *
 * More detail about these sub namespaces and their classes can be found in the documentation per sub-namespace or per class.
 *
 ...
 * * When needed, its own PSR-4 autoloader to circumvent the fact that many webshop/CMS systems still live in the pre PSR-4 autoloading era, meaning that if an autoloader already exists it often won't work with the PSR4 standard.
 * * Its own translation system to present user facing texts in this library in english and dutch, see {@see \Siel\Acumulus\Helpers\Translator} for more information about translation.
 * * Its own way of defining forms, and subsequently rendering them or mapping them to the webshop's form system, see {@see \Siel\Acumulus\Helpers\Form}, {@see \Siel\Acumulus\Helpers\FormRenderer}, and {@see \Siel\Acumulus\Helpers\FormMapper} for more information about form handling.
 * * Dependency injection: our container uses a namespace based (instead of configuration based) way of finding an instantiating the right class and it knows what arguments the constructors want,
 *   see {@see \Siel\Acumulus\Helpers\Container} for more information about our container.
 * * Support friendly: lots of logging, emailing, clear and explicative user messages, built in dry-run, allows easy use of the API test mode.
 *
 ...
 *     - Event triggered: the library currently supports the following types of events: order creation or status change, refund creation, (shop) invoice creation or (shop) invoice send to customer (if the webshop supports its own notion of invoices as clearly separate entities).
 *       This support consists of: the configuration form allows to define at what order statuses and/or what events the invoice should be sent and to
 *       handle the status change to see if the invoice has to be sent and has not already be sent.
 *     - Manual: the library defines a batch send form where the user can define a range of orders or refunds to send to Acumulus as well as some other options to influence if and how the invoices are sent.
 ...
 *   {@see \Siel\Acumulus\Invoice} for more detail about the creation and completion phases (todo).
