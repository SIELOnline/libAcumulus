 ...
 * * explicit install or update code, though it does contain code that can be called by installing or updating code form the system it resides in.
 *
 ...
 * easily usable with other webshop/CMS software systems as well. The code of these extensions can also be found in this namespace under Siel\Acumulus\{webshop}
 *
 ...
 * --------------------------------------------
 * * We use our own translation, see \Siel\Acumulus\Helpers\TranslatorInterface (todo).
 ...
 *   see \Siel\Acumulus\Helpers\ContainerInterface (todo).
 ...
 *   order creation or status change, refund creation, invoice creation or invoice send to customer (if the webshop supports its own notin of invoices as clearly separate entities).
 ...
 * * Invoice creation: if the invoice shuod be sent a raw version of it has to be created. This raw version is based on the shop order and should be in the
 *   approximate format as Acumulus expects it to receive. This will be the most work when developing a new extension. See \Siel\Acumulus\Invoice for more detail (todo).
 ...
 *   and is thus already part of the library. In rare case you may have to override classes that participate inthe completion phase to cater for specific situations.
 ...
 *   to prevent sending it again, if not a mail is snet informing the admin about any errors and/or warnings.
 *
 * Developing a new extension
 * --------------------------
 * If you want to develop an extension for another webshop these are the steps to perform:
 ...
 *     - Initialization ocde that adds the library's namespace to the webshop autoloader, or, if no autoloader is present in the webshop or it cannot be used (not PSR4 compliant),
